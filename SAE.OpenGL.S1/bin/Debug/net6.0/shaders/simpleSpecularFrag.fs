...
vec3 specularReflection(vec3 specularColor, float factor, vec3 lightColor, float hardness, vec3 viewDirection, vec3 reflectionDirection)
{
   ...
   vec3 i = pow(dotViewReflection, hardness) * factor * specularColor;
...
void main()
{
   ...
   vec3 specularColor = texture2D(specularSampler, fs_in.uv).rgb;
   specularColor = normalize(specularColor);
   ...
   vec3 specular = specularReflection(specularColor, 1.0f,pointLightColor, hardness, viewDirection, reflectionDirection);
